...
export function providingFunction()
{
  ...
  return class CatkinBuildProvider extends EventEmitter {
    ...
    {
      super();
      atom.config.observe('build-catkin-tools.cmakelists', (cmakelists) => {
      ...
      atom.config.onDidChange('build-cmcatkin-toolsake.cmakelists', () => { this.emit('refresh'); });
    ...
    createCatkinBuildTarget(args)
    {
      return {
        ...
        name : 'catkin build',
        exec : this.source_exec + this.executable,
        args : args,
